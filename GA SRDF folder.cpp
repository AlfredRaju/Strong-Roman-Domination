...
vector<int> minm(vector<int> v){                    //min and max pos
...
int RAO(vector<vector<int>> G, int itr) {       //heuristic results
    ...
    //for(int i=0;i<insize;i++){
     vector<vector<int>> inpop=heuristic(G,insize);
    //vector<int> label=heu(G);
    // for(int j=0;j<n;j++){
    //     inpop[i][j]=label[j];
    // }
//}
    ...
    // inpop = feasibility(inpop,insize,G);
    inpop = selcross(inpop,insize,G,itr);
    ...
    // cout<<"best is "<< *min<<endl;
    int q = *min;
    
    return q;                                         // Return your result
}
...
int main()
{
    ...
    string inputFolder = "C:\\Users\\alfre\\OneDrive\\Desktop\\phd\\Graphs\\New folder2";
    string outputFile = "C:\\Users\\alfre\\OneDrive\\Desktop\\phd\\Graphs\\cestout\\DErdf3.txt";
